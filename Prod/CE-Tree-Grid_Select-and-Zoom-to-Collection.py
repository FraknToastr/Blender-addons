...
bl_info = {
    ...
    "description": "Imports trees into a CE-aligned grid with Outliner focus, validation, and import confirmation.",
...
# --- Helpers ---
def is_ce_tree_collection(coll):
...
# --- Focus view and outliner ---
def get_all_objects_in_collection(coll, recursive=True):
...
def focus_view_on_collection(coll):
    ...
    # Focus in Outliner (first only)
    for area in screen.areas:
        if area.type == "OUTLINER":
            try:
                with bpy.context.temp_override(area=area):
                    bpy.ops.outliner.tree_collapse_all()
...
class CETreeProperties(bpy.types.PropertyGroup):
    ...
    reverse_rows: bpy.props.BoolProperty(name="Reverse Row Order", default=False)
    ...
    tree_selector: bpy.props.EnumProperty(
        name="Jump to Tree",
        description="Frames and focuses the selected CE Tree Grid collection",
...
class CE_OT_import_grid(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for tree_index, style_dict in tree_files.items():
            ...
            tree_collection["ce_tree_grid"] = True
    ...
    def invoke(self, context, event):
        existing = [c for c in bpy.data.collections if is_ce_tree_collection(c)]
        if existing:
